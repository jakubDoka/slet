    ...
    return struct {
        ...
        pub fn getPtr(self: *Self, id: Id) ?GetType {
            ...
            return switch (slot.header.taken.tag) {
                inline else => |t| {
                    const VarTy = std.meta.FieldType(E, t);
                    const FieldEnum = std.meta.FieldEnum(AppendBackRef(VarTy));
                    const arch = @field(self.entities, @tagName(t)).slice();
                    var ptrs: MapStruct(VarTy, mappers.AsPtr) = undefined;
                    inline for (std.meta.fields(VarTy)) |field| {
                        const fieldVar = comptime std.meta.stringToEnum(FieldEnum, field.name).?;
                        @field(ptrs, field.name) = &arch.items(fieldVar)[slot.header.taken.index];
                    }
        ...
        pub fn queryOne(self: *Self, id: Id, comptime Q: type) ?QueryOneRes(Q) {
            ...
            return switch (slot.header.taken.tag) {
                inline else => |t| {
                    const VarTy = std.meta.FieldType(E, t);
                    const FEnum = std.meta.FieldEnum(AppendBackRef(VarTy));
                    const arch = @field(self.entities, @tagName(t)).slice();
                    ...
                    inline for (std.meta.fields(Q)) |f| {
                        ...
                        const fname = (comptime std.meta.stringToEnum(FEnum, f.name)) orelse return null;
                        @field(ptrs, f.name) = &arch.items(fname)[slot.header.taken.index];
                    }
        ...
        pub fn query(self: *Self, comptime Q: type) Query(Q) {
            ...
            inline for (q.to_visit, &q.sub_queries) |field, *sq| {
                const Field = AppendBackRef(std.meta.FieldType(E, field));
                const FEnum = std.meta.FieldEnum(Field);
                const arch = @field(self.entities, @tagName(field)).slice();
                inline for (std.meta.fields(QT.QNoStats), sq.ptrs[0 .. sq.ptrs.len - 1]) |f, *ptr| {
                    const fname = comptime std.meta.stringToEnum(FEnum, f.name).?;
                    ptr.* = @ptrCast(arch.items(fname).ptr);
                }
            ...
            return struct {
                ...
                fn matches(comptime F: type) bool {
                    for (std.meta.fields(Q)) |field| {
                        if (field.type == StatsPtr) continue;
