...
pub fn World(comptime Ents: type) type {
    ...
    return struct {
        ...
        pub fn add(self: *Self, value: anytype) Id {
            const arch = comptime findArch(@TypeOf(value));
        ...
        pub fn Query(comptime Q: type) type {
            ...
            for (@typeInfo(Ents).Union.fields, &names) |f, *n| {
        ...
        pub fn remove(self: *Self, ent: Id) bool {
            const index, const kind = self.freeId(RawId.fromId(ent)) orelse return false;
            switch (kind) {
                inline else => |t| {
        ...
        fn findArch(comptime T: type) [:0]const u8 {
            return for (@typeInfo(Ents).Union.fields) |f| {
                if (f.type == T) return f.name;
            } else @compileError("ent list does not contain " ++ @typeName(T));
        }
